...
##用于生成具有判别性的对抗性图学习（增强图-用于节点全局/局部互信息最大化
...
def Cal_Feature_Net(adata, k=20, mode= "distance", metric="minkowski", include_self=False):##以距离对基因表达数据进行图建模
    print("Begin calculate gene feature graph")
...
###构建原型学习后的图像特征矩阵
def Cal_Image_Net(adata, k=20, mode= "distance", metric="cosine", include_self=False):##以距离对基因表达数据进行图建模
    print("Begin calculate image feature graph")
    image_graph = kneighbors_graph(adata.obsm['img_feat'], k, mode=mode, metric=metric,
                                            include_self=include_self)
    image_graph = image_graph+image_graph.T
    image_graph.data = np.minimum(image_graph.data, 1)
    # feature_graph = preprocess_graph(feature_graph)
    adata.obsm['image_adj'] = image_graph
    print(image_graph)
